class Campeon {
	...
	var property cantidadDeDinero		= 	0				//num 
	var   		 inventario				= 	[]				//lista

	
	//stats
	
	method vida				() 			= 	puntosDeVida 	 + self.vidaExtra() 
	method ataque			() 			= 	puntosDeAtaque + self.ataqueExtra()
   ...
   //inventariado
	method equiparItem		(unItem) 	{	
											...
											unItem.equipItem(self)
										}
	method desequiparItem	(unItem) 	{	
											inventario.remove(unItem)
											unItem.unequipItem(self)
										}
	method inventario		()			= 	inventario 		//agregado para usar en soporte
	
	//compra
	method comprarItem		(unItem)	{	
											if(self.tieneDinero(unItem.valor())) 
												self.equiparItem(unItem) 
		                                        cantidadDeDinero -= unItem.valor()
										} 
	method venderItem		(unItem)	{	
											cantidadDeDinero += unItem.valor()/2
											self.desequiparItem(unItem)
										}
}
